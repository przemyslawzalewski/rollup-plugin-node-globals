...
function nodeGlobals(options) {
  ...
  return {
    load: function load(id) {
      if (dirs.has(id)) {
        var path = dirs.get(id);
        var sanitized = JSON.stringify(path);
        return 'export default ' + sanitized;
      }
    },
    resolveId: function resolveId(importee, importer) {
      var fromDirectoryName = importee === DIRNAME;
      var fromFileName = importee === FILENAME;

      if (fromDirectoryName || fromFileName) {
        var id = randomBytes(15).toString('hex');
        var relativePath = resolve(basedir, importer);
        var resolved = fromDirectoryName ? dirname(relativePath) : relativePath;
